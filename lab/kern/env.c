...
// 将“envs”中的所有环境标记为空闲，将其env_id设置为0，并将它们插入env_free_list中
void env_init(void)
{
	// Set up envs array
...
// 为环境 e 初始化内核虚拟内存布局，成功返回 0，错误返回 < 0。
static int
...
// 分配并初始化一个新环境。成功时，新环境存储在 *newenv_store 中，成功返回 0，失败返回 < 0。
...
// 解析ELF头,将ELF二进制映像中的所有可加载段正确的加载到环境的用户内存中，设置好程序入口、堆栈、状态
static void
